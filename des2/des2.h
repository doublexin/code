...
*         0 表示加密 1 表示解密
*/
BOOL DES2::CDesEnter(LPCBYTE in, LPBYTE out, int datalen, const BYTE key[8], BOOL type)
{
	//判断输入参数是否正确，失败的情况为： 
	//!in： in指针（输入缓冲）无效 
	//!out： out指针（输出缓冲）无效 
...
*     key 8个字节的验校密码。
*/
BOOL DES2::CDesMac(LPCBYTE mac_data, LPBYTE mac_code, int datalen, const BYTE key[8])
{
	//判断输入参数是否正确，失败的情况为： 
...
*     BYTE out[8] 输出的结果字符串
*/
void DES2::XOR(const BYTE in1[8], const BYTE in2[8], BYTE out[8])
{
	for (int i = 0; i < 8; i++)
		out[i] = in1[i] ^ in2[i];
}
...
*     BYTE bit[8] 输出的转换结果
*/
LPBYTE DES2::Bin2ASCII(const BYTE byte[64], BYTE bit[8])
{
	for (int i = 0; i < 8; i++)
	{
...
*/
LPBYTE DES2::ASCII2Bin(const BYTE bit[8], BYTE byte[64])
{
	for (int i = 0; i < 8; i++)
	for (int j = 0; j < 8; j++)
...
*/
void DES2::GenSubKey(const BYTE oldkey[8], BYTE newkey[16][8])
{
	int i, k, rol = 0;
...
*     BYTE e_bit[8] 输出的密文
*/
void DES2::endes(const BYTE m_bit[8], const BYTE k_bit[8], BYTE e_bit[8])
{
	//换位表IP 
	int ip[64] = {
...
*/
void DES2::undes(const BYTE m_bit[8], const BYTE k_bit[8], BYTE e_bit[8])
{
	//换位表IP 
	int ip[64] = {
		58, 50, 42, 34, 26, 18, 10, 2,
...
*/
void DES2::SReplace(BYTE s_bit[8])
{
	int p[32] = {
		16, 7, 20, 21,
		29, 12, 28, 17,
